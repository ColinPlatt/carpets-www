...
function Cloud({
  ...
  const clouds = React.useMemo(() => [...new Array(segments)].map((_, index) => ({
    ...
    scale: 0.4 + Math.sin((index + 1) / segments * Math.PI) * ((0.2 + Math.random()) * 30),
  ...
  useFrame(state => {
    ...
    return (_group$current = group.current) == null ? void 0 : _group$current.children.forEach((cloud, index) => {
      cloud.children[0].position.x < -1000 ? cloud.children[0].position.x+=2000 : cloud.children[0].position.x -= 5;
